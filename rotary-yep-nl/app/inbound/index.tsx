...
export default function InboundScreen() {
  ...
  const sandraCools = longTermContacts.find(
    (contact) => contact.name === "Sandra Cools-Wemer",
  );
        ...
        <View style={styles.programContent}>
          ...
          >
            <FontAwesome5
              ...
              size={22}
              color={
                item.enabled ? themeColors.primary : themeColors.textTertiary
              }
            />
          </View>
          ...
          <Ionicons
            ...
            size={20}
            color={
              item.enabled ? themeColors.textTertiary : themeColors.textDisabled
            }
          />
        </View>
      </Pressable>
  ...
  const IntroSection = useCallback(
    () => (
      <View style={styles.introContainer}>
        <Text style={[styles.introTitle, { color: themeColors.primary }]}>
          Inbounds
        </Text>
    ...
    ({ item }: { item: { type: string; title?: string; item?: any } }) => {
      switch (item.type) {
        case "intro":
          return <IntroSection />;
        case "sectionHeader":
          return item.title ? <SectionHeader title={item.title} /> : null;
        case "program":
